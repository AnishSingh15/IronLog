...
function formatWeight(lbs: number): string {
  const kg = Math.round(lbs * 0.453592 * 10) / 10; // Round to 1 decimal place
...
const workoutHistory = [
  ...
  },
  {
    ...
    exercises: [
      ...
      { name: 'Seated Row (Bil)', sets: [{ w: 70, r: 10 }], description: 'Bilateral row opener.' },
      {
  ...
  },
  {
    ...
    exercises: [
      ...
      {
        name: 'Cable Lateral Raise (R)',
        sets: [{ w: 15, r: 7 }],
        description: 'One-arm lateral, 6-10 reps.',
      },
      {
        name: 'Cable Lateral Raise (L)',
        sets: [{ w: 15, r: 7 }],
        description: 'One-arm lateral, 6-10 reps.',
      },
      {
        name: 'Reverse Pec Deck (Bil)',
        sets: [
          { w: 40, r: 12 },
          { w: 40, r: 12 },
        ],
        description: 'Rear delts. 12-15 reps.',
      },
      {
        name: 'Reverse Pec Deck (R)',
        sets: [
          { w: 30, r: 13 },
          { w: 20, r: 5 },
        ],
        description: 'Rear delts single-arm.',
      },
      {
        name: 'Reverse Pec Deck (L)',
        sets: [
          { w: 20, r: 13 },
          { w: 30, r: 5 },
        ],
...
async function injectWorkoutHistoryViaAPI() {
  try {
    ...
    for (const workout of workoutHistory) {
      ...
      // Create workout day with accurate date (your workout was at 5 AM UTC on these dates)
      ...
      const createWorkoutDayResponse = await fetch(`${API_BASE}/workouts/workout-day`, {
        ...
        body: JSON.stringify({
          date: workoutDateTime, // Accurate timestamp for your workout
          completed: true,
        }),
      });
    ...
    console.log(`   - Dates: June 18, 20, 21, 2025 (accurate to your workout schedule)`);
    console.log(`   - Weight format: All weights stored in pounds (lbs) with kg conversions shown`);
    ...
    console.log(`\n💪 Weight Examples:`);
    console.log(`   - Barbell Row: ${formatWeight(112)} (your heaviest lift)`);
    console.log(`   - Incline Barbell Press: ${formatWeight(70)} (your chest work)`);
    console.log(`   - All data preserved in pounds as recorded in your original workouts`);
  } catch (error) {
