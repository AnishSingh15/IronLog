...
async function injectWorkoutHistoryViaAPI() {
  try {
    ...
    const workoutDaysResponse = await fetch(`${API_BASE}/workout-days`, {
      headers: {
        'Authorization': `Bearer ${token}`
      }
    });
    ...
    for (const workout of workoutHistory) {
      ...
      // Create workout day
      const createWorkoutDayResponse = await fetch(`${API_BASE}/workout-days`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
      ...
      for (const exercise of workout.exercises) {
        ...
        for (let setIndex = 0; setIndex < exercise.sets.length; setIndex++) {
          ...
          if (createSetResponse.ok) {
            console.log(`  ✅ Created set ${setIndex + 1} for ${exercise.name}: ${set.w}lbs x ${set.r} reps`);
          } else {
